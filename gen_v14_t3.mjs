/** gen_v14_t3.mjs --id=T3 [--n=8] [--account=geminiv14]
 * 元絵(refs/I-T3_ref.png)を参照画像として添付し、構図・力の向き・接触点・機種を厳守して
 * 博展業態へ文脈調整した写実写真を生成。スパークル/ロゴ/流血なし。
 * I-T3 機序: テールゲートリフター(昇降板)が後傾し、その上の平板状の荷(展示パネル)が
 *   傾斜面に沿って滑落し、昇降板の下／後方にいた作業者がその下敷きになる瞬間。
 * 出力: v14/T3/cand/cNN.png（候補）。最良3枚は後段で 01〜03.png に確定。 */
import path from "node:path"; import fs from "node:fs";
const arg=(k,d)=>{const a=process.argv.find(x=>x.startsWith(`--${k}=`));return a?a.split("=")[1]:d;};
const ID=arg("id","T3"), N=parseInt(arg("n","8"),10), ACCT=arg("account","geminiv14");
...
const DESC={
 T3:"At the rear of a Japanese box truck fitted with a folding TAIL-LIFT (tailgate lifter / power-gate) platform, at an exhibition-venue loading dock. The tail-lift platform is TILTED REARWARD / DOWNWARD into a slope (the platform is not level — its outer edge is lower, forming an inclined ramp surface). A large FLAT RECTANGULAR EXHIBITION DISPLAY PANEL (a heavy flat board, like a wall panel) was resting on the sloped platform and is now SLIDING DOWN the incline. A worker who was standing BELOW / BEHIND the platform is being PINNED / CRUSHED UNDERNEATH the sliding flat panel as it slides off and falls onto him — he is on the ground being trapped under the flat board. Contact point: the underside of the sliding flat panel pressing down onto the worker who is beneath/behind the tilted platform. Force direction: the flat panel sliding DOWN along the rearward-tilted platform incline and toppling/falling onto the worker, crushing him under the board. Keep this exact geometry — a flat display panel sliding off a rear-tilted tail-lift and pinning the worker underneath it.",
};
const APPROACH=[
 "Camera: true side view (side elevation), standard 35mm lens, even soft daylight, clearly showing the panel sliding down the tilted platform onto the worker beneath.",
 "Camera: from diagonally behind the truck and slightly elevated, overcast soft light, looking down along the sloped platform at the panel sliding onto the worker.",
 "Camera: a low angle near ground level looking up at the rear-tilted platform and the flat panel sliding/falling toward the camera onto the pinned worker, slightly wide lens, directional afternoon light.",
];
...
function prompt(i){
 return "The attached image is the OFFICIAL accident-prevention diagram (line illustration) of a flat board/panel sliding and crushing a worker underneath. Use it STRICTLY as the composition / layout / force-direction / contact-point reference: keep the SAME mechanism (a large flat panel/board sliding off and pinning a worker under it) but place it on a TRUCK TAIL-LIFT (power-gate) that is tilted rearward, and keep the SAME contact point (the flat panel coming down onto the worker beneath). Recreate it as a PHOTOREALISTIC photograph (do NOT keep the drawing/line-art style — make it a real photo). "
