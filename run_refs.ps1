...
$Worker = "Read RULES_REF.md and BACKLOG_REF.md in this folder. Do ONLY the single topmost unchecked '- [ ]' task, strictly following RULES_REF.md (never delete or overwrite, never use rm/Remove-Item/del/move, additions use new filenames, never kill Chrome, never print credential or API key values; do NOT generate images in this phase; before starting check existing files under refs2/ and only add what is missing). When the task is complete, change its line to '- [x]' and git add/commit/push to origin main. If no unchecked task remains, create an empty file named DONE_REF.flag and stop. If you hit a usage or rate limit, save and push work in progress, then stop."
New-Item -ItemType Directory -Force -Path ".\runner_logs" | Out-Null
$cycle = 0
while (-not (Test-Path ".\DONE_REF.flag")) {
    $cycle++; $ts = Get-Date -Format "yyyyMMdd_HHmmss"; $log = ".\runner_logs\ref_$ts.log"
    Write-Host "[$ts] cycle $cycle start"
    & claude -p --model $Model --dangerously-skip-permissions $Worker *>&1 | Tee-Object -FilePath $log
    if (Test-Path ".\DONE_REF.flag") { Write-Host "All done."; break }
