/** search_rt9.mjs : RT9「重量物で昇降板が耐えられず落下」の事故イラスト収集。
...
const OUT="C:/Users/kanet/20260522/safe1/refs2/RT9/_cand"; fs.mkdirSync(OUT,{recursive:true});

const exist=fs.readdirSync(OUT).map(f=>/^d(\d+)\./.exec(f)).filter(Boolean).map(m=>+m[1]);
let n=exist.length?Math.max(...exist):0;

const QUERIES=[
  // clipart寄り(イラスト): 重量物で昇降板が耐えられず落下／破損
  ["テールゲートリフター 重量物 過積載 昇降板 落下 災害 イラスト",true],
  ["パワーゲート 昇降板 積載超過 破損 落下 事故 イラスト",true],
  ["テールゲートリフター 昇降板 耐荷重 超過 落下 被災 イラスト",true],
  ["昇降板 重い荷 たわみ 破損 落下 作業者 下敷き 災害事例 イラスト",true],
  ["テールゲートリフター 油圧 故障 昇降板 落下 はさまれ イラスト",true],
  ["トラック 昇降装置 重量物 落下 作業者 被災 災害 イラスト",true],
  // photo含む(災害事例DBの図はphoto判定されがち)
  ["テールゲートリフター 重量物 昇降板 落下 災害事例",false],
  ["職場のあんぜんサイト テールゲートリフター 昇降板 落下 重量物",false],
  ["テールゲートリフター 死亡災害 昇降板 落下 過積載",false],
  ["tail lift overload platform collapse drop heavy load worker accident illustration",false],
];
