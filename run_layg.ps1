...
$Worker = @'
Read RULES_LAYG.md and BACKLOG_LAYG.md in this folder. Do ONLY the single topmost unchecked task line, strictly following RULES_LAYG.md. KEY RULES: never delete or overwrite any existing file (additions use NEW filenames; existing layers_v18 files may be REUSED but not destroyed); never use rm/Remove-Item/del/move/taskkill; never kill Chrome; do NOT use OpenAI (Google only = gemini-3-pro-image-preview); read GEMINI_API_KEY from .env and NEVER print key values. Generate with: node gen_img.mjs google OUTPATH.png "PROMPT" . Make parts on a pure white background then cut to transparent (try once: py -m pip install rembg onnxruntime ; else: py cutout_white.py IN OUT). Self-check thumbnails for white fringe or holes, regenerate a clearly bad one at most once. Build sample_layers_v18.pptx with python-pptx placing each part as a SEPARATE movable and rotatable picture object over a full-bleed background, for 2 situations (TGL fall / TGL load-collapse); do NOT flatten; add the note that this is a layer draft to adjust in PowerPoint; convert to PDF via PowerPoint COM. For DEPLOY, union ALL past review tokens so every existing URL stays 200 and root is 404. When the task is complete, change its line to checked and git add/commit/push origin main (commit master, push, then fast-forward master:main; if rejected pull --rebase then push). If no unchecked task remains, create an empty file named DONE_LAYG.flag and stop. If you hit a usage or rate limit, save and push work in progress, then stop.
'@
$cycle = 0
