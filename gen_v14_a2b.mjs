...
const APPROACH=[
 "Camera: TRUE SIDE VIEW (pure side elevation, profile), standard 35mm lens, even soft daylight. The worker's head and neck are UNMISTAKABLY TRAPPED and crushed in the closed gap between the basket top guard rail and the underside of the overhead beam — the gap is fully pinched shut on the worker's head. Show the side profile of the head wedged between rail and beam.",
 "Camera: TRUE SIDE VIEW (side elevation, profile), 50mm lens, soft venue light. The raised basket has pressed the worker's upper body UP against the overhead beam so the head/shoulder are clamped between the basket rail and the beam underside — clearly a crushing pinch, not someone standing or working.",
 "Camera: TRUE SIDE VIEW (side elevation), slight telephoto, hall lighting. The worker is bent/forced as the basket rail and the overhead structure close on his head and neck — the trapped contact point between rail and beam is the clear subject.",
];
