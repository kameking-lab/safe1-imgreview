/** search_rt4.mjs : RT4「昇降板上でカゴ車/台車が倒れ激突」の事故イラスト収集。
...
const OUT="C:/Users/kanet/20260522/safe1/refs2/RT4/_cand"; fs.mkdirSync(OUT,{recursive:true});

const exist=fs.readdirSync(OUT).map(f=>/^d(\d+)\./.exec(f)).filter(Boolean).map(m=>+m[1]);
let n=exist.length?Math.max(...exist):0;

const QUERIES=[
  // clipart寄り(イラスト): カゴ車/台車が昇降板上で倒れ作業者に激突
  ["テールゲートリフター カゴ車 転倒 激突 災害 イラスト",true],
  ["テールゲートリフター 昇降板 台車 倒れる 作業者 事故 イラスト",true],
  ["パワーゲート カゴ台車 転倒 下敷き 災害 イラスト",true],
  ["昇降板 ロールボックスパレット 転倒 激突 イラスト",true],
  ["トラック 昇降装置 カゴ車 倒れ 挟まれ 災害事例 イラスト",true],
  ["テールゲートリフター 台車 横転 作業者 激突 イラスト",true],
  // photo含む(災害事例DBの図はphoto判定されがち)
  ["テールゲートリフター カゴ車 転倒 災害事例",false],
  ["職場のあんぜんサイト テールゲートリフター カゴ車 転倒 事例",false],
  ["テールゲートリフター 死亡災害 ロールボックスパレット 転倒",false],
  ["tail lift roll cage tipped over crushed worker accident illustration",false],
];
