/** sheet_ra1b.mjs : refs/cand_A1（旧フェーズ候補 z01..）のコンタクトシート。画像生成なし・追記専用。*/
...
const DIR="C:/Users/kanet/20260522/safe1/refs/cand_A1";
const OUTDIR="C:/Users/kanet/20260522/safe1/refs2/RA1/_cand";
const files=fs.readdirSync(DIR).filter(f=>/^z\d+\.(jpg|jpeg|png|gif|webp)$/i.test(f)).sort((a,b)=>(+/z(\d+)/.exec(a)[1])-(+/z(\d+)/.exec(b)[1]));
...
for(let s=0;s<files.length;s+=per){
  ...
  const out=path.join(OUTDIR,`_sheetB_${sheet}.png`);
