/** search_ra6b.mjs : RA6 第2弾。高所作業車のブーム/作業床が電線に接触し「感電」が起きている事故イラストを追加収集。
 *  3要素必須: 機械=高所作業車 / ブーム・作業床が架空電線に接触し感電 / 搭乗者(地上作業者)が感電・被災。
 *  instagram-automation方式・別プロファイル(anzen)・Chrome killしない・画像生成なし・追記専用(dは既存最大値から連番)。*/
...
const QUERIES=[
  ["高所作業車 感電 作業者 バケット 電撃 死亡災害 イラスト",true],
  ["高所作業車 架空配電線 接触 感電 ビリビリ 注意喚起 イラスト",true],
  ["高所作業車 ブーム 電線 接触 作業者 感電 黒焦げ KYT イラスト",true],
  ["高所作業車 電路 接触 感電 漫画 警告 安全 イラスト",true],
  ["bucket truck aerial lift power line contact worker electrocuted cartoon safety poster",true],
  ["IPAF andy access keep clear overhead cables electrocuted worker MEWP poster",true],
  ["boom lift platform touches power line worker electrocuted shock illustration cartoon",true],
  ["cherry picker overhead line electrocution worker shocked warning illustration",true],
  // photo含む(災害事例DBの図)
  ["高所作業車 感電 災害事例 イラスト 厚生労働省",false],
  ["高所作業車 感電 死亡 災害事例 図 電線",false],
];
...
const prevMeta=fs.existsSync(path.join(OUT,"_all_meta.json"))?JSON.parse(fs.readFileSync(path.join(OUT,"_all_meta.json"),"utf8")):[];
const seen=new Set(prevMeta.map(m=>m.murl)); const uniq=[];
...
console.log(`NEW UNIQUE: ${uniq.length}  -> _all_meta_b.json`);
