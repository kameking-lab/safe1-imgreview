/** gen_v14.mjs --id=T1 [--n=6]
 * 元絵(refs/I-{ID}_ref.*)を参照画像として添付し、構図・力の向き・接触点・機種を厳守して
 * 博展業態に文脈調整した写実写真を生成。スパークル/ロゴ/流血なし。
 * 出力: v14/{ID}/cand/cNN.png（候補）。最良3枚は後段で 01〜03.png に確定。 */
import path from "node:path"; import fs from "node:fs";
const arg=(k,d)=>{const a=process.argv.find(x=>x.startsWith(`--${k}=`));return a?a.split("=")[1]:d;};
const ID=arg("id","T1"), N=parseInt(arg("n","6"),10), ACCT=arg("account","gemini");
...
const refFile=fs.readdirSync(refDir).find(f=>/^I-/.test(f) && f.startsWith(`I-${ID}_ref.`) && /\.(jpg|jpeg|png|gif)$/i.test(f));
...
const DESC={
 T1:"At the rear of a Japanese box truck fitted with a folding TAIL-LIFT (tailgate lifter) lowered/partway, at an exhibition-venue loading dock. A worker was handling a tall wheeled ROLL-CAGE cart loaded with flat EXHIBITION DISPLAY PANELS on the tail-lift platform. The worker is NOW FALLING off the OPEN OUTER EDGE of the tail-lift platform, losing balance and toppling head/shoulder-first DOWNWARD to the ground beside the truck. Contact point: the open outer edge of the tail-lift platform. Fall direction: from the platform edge straight DOWN to the ground. Keep this exact geometry — a man pitching off the open edge of the tail-lift, the roll-cage of panels still on the platform.",
};
const APPROACH=[
 "Camera: true side view (side elevation), standard 35mm lens, even soft daylight.",
 "Camera: from diagonally behind the truck and slightly elevated, overcast soft light.",
 "Camera: a low angle near ground level looking up toward the platform edge, slightly wide lens, directional afternoon light.",
];
...
function prompt(i){
 return "The attached image is the OFFICIAL accident-prevention diagram (line illustration). Use it STRICTLY as the composition / layout / force-direction / contact-point reference: keep the SAME direction of fall and the SAME contact point (platform edge) and the SAME machine (truck tail-lift) as the diagram. Recreate it as a PHOTOREALISTIC photograph (do NOT keep the drawing/line-art style — make it a real photo). "
