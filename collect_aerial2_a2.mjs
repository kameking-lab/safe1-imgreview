/** collect_aerial2_a2.mjs : A2「墜落・転落」高所作業車の事故/危険イメージ画像 収集。
 ...
 *  雛形 safe1/search_ra1.mjs 準拠（a.iusc解析・Referer fetch・拡張子中身判定）。*/
...
// --- A2 墜落・転落 クエリ（[query, clipart寄りか, 種別ラベル]）---
const QUERIES = [
  ["高所作業車 墜落 災害 イラスト", true,  "教育イラスト"],
  ["高所作業車 転落 事故 イラスト", true,  "教育イラスト"],
  ["高所作業車 バケット 墜落 安全帯 未使用 イラスト", true, "教育イラスト"],
  ["高所作業車 乗り出し 墜落 ヒヤリハット イラスト", true, "ヒヤリハット挿絵"],
  ["高所作業車 アウトリガー 墜落 KYT 教材", true, "KYT教材"],
  ["高所作業車 バケット 身を乗り出す 転落 危険 イラスト", true, "教育イラスト"],
  ["高所作業車 墜落 注意喚起 ポスター", true, "注意喚起ポスター"],
  ["高所作業車 安全帯 命綱 未着用 墜落 イラスト", true, "教育イラスト"],
  ["高所作業車 墜落 死亡災害 事例 厚生労働省", false, "実事例図/写真"],
  ["職場のあんぜんサイト 高所作業車 墜落 災害事例", false, "実事例図/写真"],
  ["高所作業車 転落 災害事例 建災防", false, "実事例図/写真"],
  ["高所作業車 墜落 災害事例 図", false, "実事例図/写真"],
  ["aerial work platform fall from bucket accident illustration", true, "教育イラスト"],
  ["boom lift worker fall safety harness illustration", true, "教育イラスト"],
  ["scissor lift fall accident hiyari hatto illustration", true, "教育イラスト"],
  ["aerial lift fall from height worker accident poster", true, "注意喚起ポスター"],
];
...
try {
  for (const [q, clip, kind] of QUERIES) {
    ...
    if (zeroStreak >= 8) { console.log("=== 8クエリ連続 新規ゼロ → A2終了 ==="); break; }
...
console.log(`\nA2 DONE: saved=${savedTotal}  totalImgNum=${n}  queriesRun=${newQueriesDone.length}`);
