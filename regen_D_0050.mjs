// regen_D_0050.mjs — #0050 の D_google_event.png のみ再生成（元版に実在ロゴ ISUZU が出たため）。
...
const machine = "a Japanese-spec aerial work platform / MEWP (boom or scissor lift with a work basket)";
const FAITH = `Reproduce the EXACT accident mechanism of the reference illustration: same composition, same direction of force, same contact point, same machine type (${machine}). The worker falls/dangles out of the aerial work basket from height.`;
...
const NOLOGO = "ABSOLUTELY NO brand names, NO manufacturer logos, NO readable text, NO badges anywhere — the truck and all equipment must be PLAIN and UNBRANDED (no 'ISUZU', no maker text on the grille/door/body). NO blood/gore.";
...
meta.results["D_google_event.png"] = { ok: true, model: res.model, regen: "no-logo-fix(ISUZU)" };
