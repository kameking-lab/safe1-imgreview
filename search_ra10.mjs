/** search_ra10.mjs : RA10「その他 高所作業車起因の死傷（RA1〜RA9で未カバーの事故事象）」の事故イラスト収集。
 *  3要素必須: (1)機械=高所作業車(ブーム式/シザース式/トラック搭載等) (2)事象=墜落/転倒/下敷き/はさまれ/激突/落下物直撃 等が"起きている"
 *  (3)人(搭乗者または地上作業者)が被災/されかけ。RA1〜RA9で既出の事象は重複回避し、未カバーの「その他」事象を狙う:
 *   - 地上作業者がブーム/作業床と上方・周囲構造物に挟まれ(地上側)・轢かれ・激突
 *   - 機体横転で搭乗者/地上者が機体の下敷き
 *   - 作業床から工具/資材が落下し下の作業者を直撃(落下物)
 *   - 一般的な高所作業車からの墜落(原因限定なし)・無資格/単独作業中の被災 等。
...
const OUT="C:/Users/kanet/20260522/safe1/refs2/RA10/_cand"; fs.mkdirSync(OUT,{recursive:true});

const exist=fs.readdirSync(OUT).map(f=>/^d(\d+)\./.exec(f)).filter(Boolean).map(m=>+m[1]);
let n=exist.length?Math.max(...exist):0;

const QUERIES=[
  // clipart寄り(イラスト) — 未カバー事象を狙う
  ["高所作業車 地上作業者 ブーム はさまれ 災害 イラスト",true],
  ["高所作業車 横転 機体 下敷き 作業者 事故 イラスト",true],
  ["高所作業車 作業床 工具 落下 直撃 下の作業者 災害 イラスト",true],
  ["高所作業車 走行 地上作業者 激突 ひかれ 事故 イラスト",true],
  ["高所作業車 からの 墜落 災害 KYT イラスト",true],
  ["高所作業車 上昇中 構造物 はさまれ 地上 監視者 注意 イラスト",true],
  // photo含む(災害事例DBの図はphoto判定されがち)
  ["建災防 高所作業車 災害事例 はさまれ 下敷き",false],
  ["職場のあんぜんサイト 高所作業車 墜落 はさまれ 災害事例 イラスト",false],
  ["陸災防 高所作業車 落下物 激突 災害事例",false],
  ["aerial lift dropped object struck worker below accident illustration",false],
  ["MEWP crushed pinned ground worker overhead boom accident illustration",false],
  ["aerial work platform tip over worker crushed under machine illustration",false],
];
