/** search_rt5.mjs : RT5「昇降板降下中に手足を地面・床との間にはさまれ」の事故イラスト収集。
...
const OUT="C:/Users/kanet/20260522/safe1/refs2/RT5/_cand"; fs.mkdirSync(OUT,{recursive:true});

const exist=fs.readdirSync(OUT).map(f=>/^d(\d+)\./.exec(f)).filter(Boolean).map(m=>+m[1]);
let n=exist.length?Math.max(...exist):0;

const QUERIES=[
  // clipart寄り(イラスト): 昇降板の降下中に手足が地面/床との間に挟まれる
  ["テールゲートリフター 昇降板 足 はさまれ 災害 イラスト",true],
  ["パワーゲート 降下 足 挟まれ 地面 事故 イラスト",true],
  ["テールゲートリフター 昇降板 下降 手 はさまれ 災害 イラスト",true],
  ["昇降板 接地 足 挟まれ 巻き込まれ 災害 イラスト",true],
  ["トラック 昇降装置 降下 足 床 挟まれ 事故 イラスト",true],
  ["テールゲートリフター プラットフォーム 降下 はさまれ 災害事例 イラスト",true],
  // photo含む(災害事例DBの図はphoto判定されがち)
  ["テールゲートリフター 昇降板 はさまれ 災害事例",false],
  ["職場のあんぜんサイト テールゲートリフター 挟まれ 足",false],
  ["テールゲートリフター 死亡災害 はさまれ 昇降板 降下",false],
  ["tail lift platform foot crushed descending ground accident illustration",false],
];
